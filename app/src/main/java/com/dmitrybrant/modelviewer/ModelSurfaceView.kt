...
class ModelSurfaceView(context: Context, model: Model?) : GLSurfaceView(context) {
    ...
    /**
     * Apply custom per-axis scale to the model.
     * Safely queues the operation on the GL thread.
     * @param scaleX  multiplier for X axis (e.g. 2.0 = twice as wide)
     * @param scaleY  multiplier for Y axis
     * @param scaleZ  multiplier for Z axis
     */
    fun applyModelScale(scaleX: Float, scaleY: Float, scaleZ: Float) {
        queueEvent {
            renderer.applyModelScale(scaleX, scaleY, scaleZ)
        }
        requestRender()
    }
